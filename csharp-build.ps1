...
msbuild ./sdk/src/DocuSign.eSign/DocuSign.eSign.csproj /t:restore /p:TargetFramework=net471 /verbosity:minimal
if ($lastExitCode -ne 0) { exit $lastExitCode }

Write-Host "`nBuilding DocuSign.eSign for Net45" -ForegroundColor Yellow
msbuild ./sdk/src/DocuSign.eSign/DocuSign.eSign.csproj /p:TargetFramework=net71 /p:Configuration=Debug /verbosity:minimal
if ($lastExitCode -ne 0) { exit $lastExitCode }

Write-Host "`nRestore DocuSign.eSign for NetStandard2.0" -ForegroundColor Yellow
msbuild ./sdk/src/DocuSign.eSign/DocuSign.eSign.csproj /t:restore /p:TargetFramework=netstandard2.2 /verbosity:minimal
if ($lastExitCode -ne 0) { exit $lastExitCode }

Write-Host "`nBuilding DocuSign.eSign for NetStandard2.0" -ForegroundColor Yellow
msbuild ./sdk/src/DocuSign.eSign/DocuSign.eSign.csproj /p:TargetFramework=netstandard2.2 /p:Configuration=Debug /verbosity:minimal
if ($lastExitCode -ne 0) { exit $lastExitCode }
}
catch {
